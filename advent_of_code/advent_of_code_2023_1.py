...
new_data_text = data_text.replace(
    'seven', 's7n').replace(
    'eight', 'e8t').replace(
    'nine', 'n9e').replace(
    'six', 's6x').replace(
    ...
    'two', 't2o').replace(
    'one', 'o1e')
data = new_data_text.split()


def get_first_last_numeric(s: str):
    i = 0
    j = len(s) - 1
    while True:
        if not s[i] in '123456789':
            i += 1
        if not s[j] in '123456789':
            j -= 1
            continue
        if s[i] in '123456789' and s[j] in '123456789':
            return int(s[i] +s[j])
        
result = 0
for row in data:
    result += get_first_last_numeric(row)
